...
export const MatchCard = component$<MatchCardProps>((props) => {
  ...
  const secondsToMinutesAndSeconds = useComputed$(() => {
    const seconds = game.game_duration;
  ...
  return (
    <div id="flex-wrapper" class={`flex ${game.win ? "bg-[#85D7FF]" : "bg-[#FF8585]"} gap-x-5 rounded-3xl mx-72 flex-nowrap`}>
      <div id="gameinfo" class="ml-5">
        ...
        />
        <div id="gameduration" class="text-center text-2xl font-semibold">{secondsToMinutesAndSeconds.value}</div>
      ...
      <div id="playerscore" class="flex flex-col gap-y-4 justify-center">
        <div id="playername" class="text-center text-5xl font-semibold">{game.name}</div>
      ...
      <div id="items" class="grid grid-cols-3 grid-rows-2">
        {items.map((item) => {
          if (item === undefined) {
            return <div></div>;
          } else {
      ...
      <div id="ward" class="inline-block mt-10">
        <img src={`https://raw.communitydragon.org/latest/plugins/rcp-be-lol-game-data/global/default/assets/items/icons2d/${ward}`} />
      </div>
    </div>
  );
});
